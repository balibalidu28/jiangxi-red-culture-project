async function loadEncyclopedias() {
    ...
    try {
        ...
        const encyclopediaTableBody = document.querySelector("#encyclopedias tbody");
        if (!encyclopediaTableBody) {
            console.error("找不到表格tbody元素");
            return;
        }

        encyclopediaTableBody.innerHTML = ""; // 清空旧表格内容
        ...
        encyclopedias.forEach(item => {
            ...
            `;
            encyclopediaTableBody.appendChild(row);
        });
...
async function deleteEncyclopedia(id) {
    ...
    if (!confirm("确定要删除这个百科条目吗？此操作不可撤销！")) {
        return;
    }
